...
The above correlations are all statistically significant[check - produce p-value table]. The most surprising result is the lack of correlation with the ROI variable. It is unclear why there is not a positive relationship relative to some of the social media metrics above. This may potentially suggest the impact of outliers in our sample, which may be reducing any underlying correlation [scope to include scatterplot conditioned on different return levels]. In contrast to revenues, which at least demonstrates positive correlation consistent with what is expected for movie facebook likes, and number of voted users, we do not consider ROI as suitable for further analysis. 
