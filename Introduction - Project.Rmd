...
film_country <- old_data %>%
  group_by(country) %>%
<<<<<<< HEAD
  summarise(mean_gross = mean(gross), n=n())
head(film_country, 20,40)
=======
  summarise(n=n())
>>>>>>> e63ba8d457207a1d51bb44b38d913f3f51af0772
...
<<<<<<< HEAD
#Looking at non-US data

filter_data_nonUS <- filter(old_data, country!="USA", language=="English")
summary(filter_data_nonUS[,"gross"])
...
The film industry is expected to generate global box office revenues of almost US$49.3bn by 2020, from a forecast of US$38.3bn in 2016[^1], representing a CAGR of over 32%. These growth levels are likely to result in vast resources being poured into the industry, in search of the next "big hit". Countries, recognising the value of developing a film industry to support a variety of local and national initiatives, have supported these activities, providing tax breaks to investors, to mitigate the material upfront costs involved.
...
To test for robustness of our regression model, we have conducted a regression analysis and t-tests for each independent variables in our previous model as shown below:
...
t4 <- t.test(data11_15$sum_actor_occurence, data16$sum_actor_occurence, alternative='two.sided') 
t5 <- t.test(data11_15$director_occurence, data16$director_occurence, alternative='two.sided') 
t6 <- t.test(data11_15$num_voted_users, data16$num_voted_users, alternative='two.sided') 
t_test_variable <- c('budget', 'action-dummy', 'scifidummy', 'sum_actor_occurence','director_occurence', 'num_voted_users')
t_test_p_value <- c(t1$p.value, t2$p.value, t3$p.value, t4$p.value, t5$p.value, t6$p.value)
t_test_table <- data.frame(t_test_variable, t_test_p_value)
...
rev_mod9 <- lm(data=data11_15, log6_gross ~ budget + actiondummy + scifidummy + sum_actor_occurence + director_occurence + num_voted_users)

log6_gross <- (log(data16$gross)^6)

rev_mod10 <- lm(data=data16, log6_gross ~ budget + actiondummy + scifidummy + sum_actor_occurence + director_occurence + num_voted_users)

stargazer(rev_mod9, rev_mod10, type = "text")
```
