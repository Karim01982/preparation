...
awards_data <- read.csv("~/IC/Data Structures and Algorithms/Group-G/Drafts/awards_metadata.csv", stringsAsFactors = FALSE, encoding = "UTF-8")
...
The film industry is expected to generate global box office revenues of almost US\$49.3bn by 2020, from a forecast of US\$38.3bn in 2016[^1], representing a CAGR of over 32%. These substantial growth levels are likely to result in vast resources being poured into the industry, in search of the next "big hit". Countries, recognising the value of developing a film industry to support a variety of local and national initiatives, have supported these activities, providing tax breaks to investors, to mitigate the material upfront costs involved.
...
After deleting the variables that have N/A values and that are less relevant to the aim of our report, our final variables are:
...
stargazer(Usereg11, Usereg12, type = 'text',report = "vct*")
```
...
stargazer(rev_mod7, rev_mod8, type = "text", report='vct')
...
stargazer(social_11_15, social_16, type = "text", report='vct*')
```
...
stargazer(commercial_11_15, commercial_16, type = "text", report='vct*')
```
...
This report seeks to examine a random sample of movie data pulled from IMDB data between 2011-16, concentrating on the US given its role in influencing the wider film industry. This will seek to understand how we should measure popularity, focusing on 2011-15 panel data, before devising a regression model that can help predict popularity. We will then test its capacity to predict 2016 trends, before noting the limitations of our work, and areas for further development.
...
###What metric(s) should we use to determine our dependent variable?
...
rev_mod4 <- lm(data=data11_15, gross ~ (budget + actiondummy + scifidummy + actor_oscars + dir_oscars + movie_oscars + 
                                          sum_actor_occurence + director_occurence))
...
stargazer(rev_mod1, rev_mod2, rev_mod3, rev_mod4, rev_mod5, rev_mod6, rev_mod7, rev_mod8, type = "html", report = "vct*")
...
```{r, results = "asis"}
log4numvoted_users <- (log(data11_15$num_voted_users)^4)
Usereg11 <- lm(data = data11_15, log4numvoted_users ~ gross + actor_oscars + familydummy + scifidummy)
log4numvoted_users <- (log(data16$num_voted_users)^4)
Usereg12 <- lm(data = data16, log4numvoted_users ~ gross + actor_oscars + familydummy + scifidummy)

stargazer(Usereg11, Usereg12, type = 'text')

```
```{r, results = "asis"}
t.test(data11_15$familydummy, data16$familydummy, alternative='two.sided') 


```
```{r, results = "asis"}
t.test(data11_15$gross, data16$gross, alternative='two.sided')  
...
```{r, results = "asis"}
t.test(data11_15$scifidummy, data16$scifidummy, alternative='two.sided') 
...
We discuss the steps taken in cleaning our underlying datasets prior to analysis:         
* **IMDB Movie Dataset**           
* **Oscar Awards Dataset**
* **Compilation of both Datasets**
