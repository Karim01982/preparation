...
for( i in 1:nrow(accuracy_k)){
   ...
   knn_i2 <- knn(train =traind, test = traind, cl = train_labels, k = i)
   knn_i3 <- knn(train =traind, test = traind, cl = train_labels, k = i)
   mean_a <-(mean(train_labels == knn_i1)+mean(train_labels == knn_i2)+mean(train_labels == knn_i3))/3
...
accuracy_k_order <- accuracy_k[order(accuracy_k[ , "misclassification_rate_v"]), ]
accuracy_k_order[1, ] # the optimal model is the model with k =24, and the mean accuracy after repeating 3 times is 0.734.



library(ggplot2)
ggplot(accuracy_k) + geom_line(aes(x = k, y = misclassification_rate_v)) +geom_line(aes(x = k, y = misclassification_rate_t)) +ggtitle('misclassification_rate for Different K')
