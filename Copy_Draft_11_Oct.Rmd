...
residualPlot(rev_mod7) # residual plot for our commercial model (with the log)
```
...
We start by regressing our 2011-15 model on our Jan-Aug 2016 dataset. This includes [37] data-points. With our log-transformed equations consistent with a normal distribution, we assume that a sample of this size should satisfy the central-limit theorem (ie. a sample > 30-35), enabling meaningful analysis; although we recognise further data-points would provide further comfort.
...
stargazer(usereg10, usereg11, rev_mod7, rev_mod8, type = 'html',report = "vct*")

```

Nominally, our models would suggest a good fit with 2016 data, based on high ${R}^{2}$ being attained. However, combined with low t-values generally being observed (with the exception of revenue and budget variables in our social media and revenue regressions respectively), this would normally be a symptom of multi-collinearity, notwithstanding our efforts to achieve the contrary. We give our models the benefit of the doubt, however we must recognise the potential for this to seriously undermine any resulting conclusion. [The material significance of our constants (i.e p values < 0.001)) may also reinforce our suspicion of an omitted variable bias - we discuss this within Section 6].

We first perform an F-test to contrast the variance between our 2011-15 and 2016 models. Should these variances be similar, enabling the rejection of $H_0$ this would indicate our 2011-15 model may represent a reasonable fit on 2016 data. Formalising our hypothesis:
...
We can apparently reject our null hypothesis for both regressions at a 5% confidence interval. Given the potential issues we have identified above, this does not seem unreasonable as an assumption, although we note for our social media regression that we could reject our alternate hypothesis at a 1% interval. It may therefore not be entirely clear, with this highlighting the prospect of a Type I error.

We separately examine what our parameter coefficients return across both models. We formulate our hypothesis as follows:

$$H_0 : \beta_{(i\ , \ 11-15)} = \beta_{(i\ ,\ 16)} \  \ \ or \  \ \ \  \beta_{(i\ ,\ 11-15)} - \beta_{(i\ ,\ 16)} = 0$$
$$H_1 : \beta_{(i\ , \ 11-15)} \neq \beta_{(i\ ,\ 16)} \  \ \ or \  \ \ \  \beta_{(i\ ,\ 11-15)} - \beta_{(i\ ,\ 16)} \neq 0$$
                       
First, we look at results of the Z test for the social popularity model:
