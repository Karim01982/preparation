...
This uncertainty is reinforced in the range of revenue figures, and corresponding budget figures respectively. With revenue, this volatility is likely to be more pronounced in practice, as our metric only focuses on box office tickets. These figures imply a median film will have a budget of US\$40m and be expected to generate box office revenues of US\$52m, broadly consistent with our ROI estimate above.
...
ggplot(data=final_A, aes(x=gross, y=num_voted_users)) +
  geom_point(position="jitter", alpha=0.5) + facet_wrap(~return) + ggtitle('Number of Voted Users based on ROI') + 
...
In contrast to revenues, we do not consider ROI as suitable for further analysis as a dependent variable. We produced two scatter plots to confirm this finding. The first, which segregates returns, shows no underlying change on the positive relationship between box office revenue and IMDB votes. In other words, each type of return appears just as likely as we vary IMDB votes and sales achieved. This is more apparent when we produce a scatterplot of revenues over returns, where no relationship appears apparent between high-grossing and small box office sales. These results are surprising, as we expected some pattern to emerge, be it positive or negative correlation (e.g. either films grossing higher box office sales generate larger returns, or small films performing far better compared to large films that lose money due to higher costs). Neither is evident, of which we comment further in section 6, as we'd expect ROI to be a significant factor in measuring success.   

Revenues as a financial metric has demonstrated the positive relationships we expected to see amongst social media (e.g. facebook likes and number of voted users), indicating that expressions of support tend to correspond with higher box office revenues (i.e. people go to the cinema). We do note the more weak positive correlation with IMDB scores (esp. relative to number of voting IMDB users), which we suspect is a function of the latter's negative skew, although given the strong correlation with budget, this partly offsets these concerns. The latter relationship would correspond well with blockbusters generating higher revenues, conditioned on budgets that can support the hiring of better actors, and increased marketing / promotional spend. 
...
We start with a simple regression against box office revenues to gauge whether a film's cinema success (in purely absolute terms) corresponds with increased IMDB activity. This regression is aimed at determining the types of films that may facilitate a positive investment decision in future.

We do not consider budget or ROI as an independent variable, given the former's high correlation (and thus introducing the risk of multi-collinearity), and the latter's minimal correlation coefficient. Our initial results (equation 1) indicate an additional IMDB vote is cast for every US\$1,000 in ticket sales, that is statistically significant at a 5% confidence interval (we assume all t-tests are two-tailed). An adjusted ${R}^{2}$ of 0.435 is generated, providing a benchmark that can be refined further, given the size of our residual standard error ("RSE").

We first examine the impact of particular actors and a director within our sample (equations 2 and 3), to address whether audiences respond to previous work produced. This is premised on counting the number of occurrences within our sample, and thus is limited in not consider other work undertaken within the period. Notwithstanding, this may help reduce any serial correlation within our error term (i.e. an actor favoured in the past is likely to remain favoured at present). From our results, only the 1st actor appears statistically significant, with a negligible impact on increasing our adjusted ${R}^{2}$, or reducing the RSE term.

As an alternative, we appraise the number of Oscars each actor and director has been awarded (equation 4), where these statistics reflect lifetime performance. This does introduce error, as we aggregate Oscars across the period, and apply this figure restrospectively (eg. an Oscar won in 2016 would be reflected in a 2012 sample point starring the actor), however we don't feel it detracts from measuring the popularity of an Oscar, which is what the variable is intended to achieve (i.e. it does not seem unreasonable to assume an Oscar is won by an actor who has featured in films over many years, building up a following and wider respect for their work). 

We find actor Oscars is stastically significant, contributing an extra 61,000 votes per Oscar, with movie Oscars insignificant. The latter is likely attributable to an extended lag between a film being released and the subsequent award ceremony being held; thus limiting its impact on box office sales. The adjusted ${R}^{2}$ increases to 0.476 with our RSE term falling, representing an improvement in fit.
...
Overall, this model should mitigate the effect of statistical bias affecting our standard errors, and is intended to attain specification efficiency. However, it is likely our model violates our i.i.d. assumptions on our error terms, which is likely to be affected by an omitted variable(s). The high t-value attained on our intercept coefficient might be suggestive of this. We discuss overall model robustness further in section 5.
...
rev_mod2 <- lm(data=data11_15, gross ~ (budget + actiondummy + adventuredummy + comedydummy + familydummy + fantasydummy + horrordummy + romancedummy +
                                          scifidummy + thrillerdummy))
rev_mod3 <- lm(data=data11_15, gross ~ budget + actiondummy + scifidummy + comedydummy + actor_oscars + dir_oscars + movie_oscars)
rev_mod4 <- lm(data=data11_15, gross ~ (budget + actiondummy + scifidummy + comedydummy + director_occurence + actor1_occurence + actor2_occurence +
                                          actor3_occurence))
...
stargazer(rev_mod1, rev_mod2, rev_mod3, rev_mod4, rev_mod5, rev_mod6, rev_mod7, type = "html", report = "vct*")
...
Moving ahead with this equation, in equation (2), we try to guage which genre movie impacts on the revenue or commercial success. In this model, we see that the adjusted ${R}^{2}$ increases to **~0.480** and there is a decrease in the RSE value. Furthermore, we observe that the genres Action genre is the most significant genre, closely followed by Sci-fi genre movies. Traditionally, these movies are known to cater to wide audience base, and also have higher budgets. Furthermore, we see the intercept for the revenue and action genre movies is a negative one, which indicates that the data represents that action movies are often loss making ones. 

Taking only the significant variables forward, we asses the interaction of the duration of the movie with the revenue in equation (3). Although we see a significance and a relation between them, we feel that duration of the movie is seldom a deciding factor for audience. Furthermore, as described by the plot above (insert table/plot number), the duration's fit can be impacted by residuals and thus we reject the significance of that variable in the model.

Since the starcast of a movie is very significant crowd puller, in equation (4) we try to gauge the impact of oscar awards for actors and directors on the revenue of the movie. Contrary to the belief, the data doesnt represent any significant impact of the actor and director oscar wins to the  movie revenue with one oscar adding only \$7million and \$2.2million repectively for actor and director oscars respectively. This can be explained by two points: first, that an award winning actor/director would be costy to hire, and secondly, the data we have is a relatively small sample.

In equations (5) to (9), we take into account the impact of actor and diretor occurence variables, individually and when taken together. When taken individually, we see that from the variables 'actor1_occurence', 'actor2_occurence' and 'actor3_occurence', only actor2_occurence is have a signicant impact on revenues, which lead us to believe that the supporting actors being good does have a positive impact on the revenue. This being said, we have to realize that this data is a recent sample, which leads us to believe that the relation can be more stronger/weaker if we increase the sample size. Furthermore, on observing the dataset, we find that the 'actor1, 'actor2', and 'actor3' columns contain names of actors at random and are not based on their significance in the movie at all. A perfect example of this is record 12 for the movie Jurassic World, where the lead actor Chris Pratt doesn't feature in any of the columns. Seeing that in general the actor occurence or popularity does impact the revenue directly, we keep the 'actor1_occurence' and 'actor2_occurence' we keep these 2 variables in the equation as determinants of revenue variable. The final equation (9) gives us an adjusted $R^2$ of 0.525 and the lowest RSE value yet.

In order to accurately build a model for revenue, we have to take into account the social popularity of the movie. Thus in equation (10) we add the variable for social population/siccess, which is 'num_voted_users' to the regression equation. The adjusted $R^2$ increases to ~0.614 and the RSE decreases significantly. Although this is not the best fit solution, with the equation holding true for 61% of our dataset, we can conclude that it is a good fit solution.

Now that we have a model that is a good fit, we aim to reduce the error in the same by reducing the error and making our regression free of bias. To do that, we use the ${\log {x}}^{6}$ transformation to take into account the skew in the revenue column of our dataset. After using the log, we observe the adjusted $R^2$ comes down to ~0.565, which still makes it a good fit and also reduces the bias of the model. 
...
As the first step of validating the robustness of the social popularity and commercial success models, we use residual plots in order to examine how does the regression models fit the data.

```{r, echo=FALSE}
residualPlot(usereg10) # residual plot for our social media model (with the log)
residualPlot(rev_mod7) # residual plot for our commercial model (without the log)


```

In addition, when taking into account the 'log' dependent variable in both social popularity and comercial sucess regression models, we have noted serial correlation in residual plots for them. This can be explained by ommitted variables in our model. This means that R-Standard Error might not be accurate and thus the fit or any test results on the models might be innacurate. 

###B. T-test on Dataset 2011-15 and 2016

In addition to the residual plots, we have conducted t-tests on the independant variables of our social medial and commercial models for the dataset of 2011-2015 and of 2016.

As T test requires the random variables to follow normal distribution, we could only use the logarithm transformed variables that were proved to be normally distributed. Besides, since the dummy variables can only be 0 or 1, they are not suitable for T test either. Therefore we will conduct T tests for normally distributed variables, and compare the summary statistics of dummy variables.

The null hypothesis for our t-tests is that there are no significant differences between the variable of the dataset of 2011-2015 and dataset of 2016. If the p-value is greater than 0.05, the null hypothesis will be accepted.
...
t2 <- t.test(data11_15$director_occurence, data16$director_occurence, alternative='two.sided') 
t3 <- t.test(data11_15$actor2_occurence, data16$actor2_occurence, alternative='two.sided') 
t4 <- t.test(data11_15$actiondummy, data16$actiondummy, alternative='two.sided') 
t5 <- t.test(data11_15$comedydummy, data16$comedydummy, alternative='two.sided') 
t6 <- t.test(data11_15$gross, data16$gross, alternative='two.sided') 
t7 <- t.test(data11_15$actor_oscars, data16$actor_oscars, alternative='two.sided') 
t8 <- t.test(data11_15$familydummy, data16$familydummy, alternative='two.sided') 
t9 <- t.test(data11_15$scifidummy, data16$scifidummy, alternative='two.sided') 


t_test_variable <- c('budget','director_occurence','actor2_occurence','actiondummy','comedydummy','gross','actor_oscars','familydummy','scifidummy')
t_test_p_value <- c(t1$p.value, t2$p.value, t3$p.value, t4$p.value, t5$p.value, t6$p.value, t7$p.value, t8$p.value, t9$p.value)
t_test_sig_level <- c('< 0.05','> 0.05', '> 0.05','< 0.05', '> 0.05','> 0.05', '> 0.05','> 0.05', '< 0.05')
...
The p-values for the director_occurence , actor2_occurence variables, gross, actor_oscars, familydummy and comedydummy are high enough (greater than 0.05) and hence we could accept the null hypothesis and conclude that there are no significant differences in those data of 2011 to 2015 and those data of 2016. However, the p-value for budget variable, acitondummy and scifydummy are less than 0.05 and far outside the confidence interval, therefore this variable is not suitable for the model application on year 2016 movies. This report believes that the reason of this inconsistent could be the limitation of the raw data. 

Below is the summary statistics of the budget variable in dataset 2011-15 and dataset 2016:
...
actiondummy_11_15 <- as.vector(summary(data11_15$actiondummy))
actiondummy_16 <- as.vector(summary(data16$actiondummy))
scifidummy_11_15 <-as.vector(summary(data11_15$scifidummy))
scifidummy_16 <-as.vector(summary(data16$scifidummy))


insig_variable_table <- data.frame(Statistics, budget_11_15, budget_16, actiondummy_11_15,actiondummy_16, scifidummy_11_15, scifidummy_16)

kable(insig_variable_table, digits = 2, caption = 'Variables')

```

As the summary statistics shown in the above table, it is very obvious that the available data of those three variables for 2016 is very different from the ones for 2011-15. It is due to the size of the dataset for 2016 as it only contains the data up to August 2016. Therefore, the report does not recommend to use the current data of these three variables to conduct future analysis.
...
stargazer(rev_mod7, rev_mod8, type = "html", report='vct')

```


The left column in the table stands for the commercial popularity model we established in the previous sections (2011 - 2015), and the right one for year 2016. The budget, number of voted users constant terms are highly significant. However, the cast occurence and actiondummy are not significant in the regression model of 2016. The adjusted ${R}^{2}$ equals 0.677 and F-test is significant which to some degree support the correctness of our model. One reason why cast of characters dose not significant is that the data for 2016 is not large enough -- In the 2016 dataset there are only 37 observations while there are 4 independent variable. The direction of most of our variables in model of 2016 are the same as model of 2011~2015, except the actor1_occurence variables, which can be explained by the lager constant variable.
...
###C. F-test on Dataset 2011-15 and 2016

```{r, echo=FALSE, results = "asis"} 
var.test(Usereg11, Usereg12) # F-test for social popularity
var.test(rev_mod7,rev_mod8 )# F-test for commecial popularity
```

Since the p-values for social media popularity regression model is smaller than 0.05, we will have to reject the null hypothesis stating that the variances are equal thus we can conclude the following:

- The regression models for social media popularity for data 2011-2015 and data 2016 are not the same.

Because the residual plot demonstrate that there is serial correlation in the regression model (For example, when people love one actors in 2011, then he is more likely to love this actor in the later year, like 2016. However, in the dataset, it just presume that people no longer love the actor in 2016), so that the standard error might be not correct , thus the result of t-test and the F-test might not be accurate too.

Since the p-value for commercial sucess regression model is larger than 0.05, we wii accept the null hyphothesis stating that the variances are equal thus we can conclude the following:

- The regression models for commercial poularity for data 2011-2015 and data 2016 are the same


###D. Z-test on the Coefficients of Models on Dataset 2011-15 and 2016

The available dataset is not as ideal for the future analysis (2016), as the previous sections explained. However, it might still be meaningful to conduct a z-test on the coefficients of the two models based on dataset 2011-15 and 2016, to see whether there is any significant difference on the same models built from two different samples.

First, we look at the social popularity model:

```{r, results = "asis", echo=FALSE}
#social:
log4numvoted_users_11_15 <- (log(data11_15$num_voted_users)^4)
social_11_15 <- lm(data = data11_15, log4numvoted_users_11_15 ~ gross + actor_oscars + familydummy + scifidummy)

log4numvoted_users_16 <- (log(data16$num_voted_users)^4)
social_16 <- lm(data = data16, log4numvoted_users_16 ~ gross + actor_oscars + familydummy + scifidummy)

stargazer(social_11_15, social_16, type = "html", report='vct*')
```
...
The left column in the regression table stands for the model that was built based on year 2011 - 2015 data, and the right one for year 2016. The gross and constant terms are highly significant, however, the actor_oscars, familydummy and scifidummy are not as significant.The adjusted ${R}^{2}$ is satisfyingly high and F-test is significant.

The z-test for the coefficients of gross and scifidummy variables are significant and we believe that there is no much difference between the effect of these two coefficients. The results, however, for actor_oscars and familydummy variables, are rejecting the null hypothesis. 

One reason why the actor_oscars and familydummy variables are not significant for 2016 data is that the current 2016 dataset is too small (only 37 observations while there are 4 independent variables) and the two datasets on these variables are not consistent based on the t test and F test we conducted previously. 

However, the signs of all coefficients in the 2016 model are the same as 2011-2015 model, which could prove the general correctness of our model.

Second, we look at the commercial success model:

```{r, echo=FALSE, results = "asis"}
#commercial:
log6_gross_11_15 <- (log(data11_15$gross)^6)
commercial_11_15 <- lm(data=data11_15, log6_gross_11_15 ~ (budget + actiondummy + comedydummy  + director_occurence + actor2_occurence))


log6_gross_16 <- (log(data16$gross)^6)
commercial_16 <- lm(data = data16, log6_gross_16 ~ (budget + actiondummy + comedydummy  + director_occurence + actor2_occurence))

stargazer(commercial_11_15, commercial_16, type = "html", report='vct*')
```
...
The left column in the regression table stands for the model that was built based on 2011 - 2015 data, and the right one for year 2016. Only the budget variable is highly significant. 

The z-test for the coefficients, however, shows a slightly more positive result. The coefficients of budget, comedy dummy and director_occurrence variables do not show much difference on dataset 2011-15 and dataset 2016. 
