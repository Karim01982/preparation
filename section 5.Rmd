...
t_test_variable <- c('budget', 'director_occurence','actor2_occurence')
t_test_p_value <- c(t1$p.value, t2$p.value, t3$p.value)
t_test_sig_level <- c('< 0.05','> 0.05', '> 0.05')
...
The p-values for the director_occurence and actor2_occurence variables are high enough (greater than 0.05) and hence we could accept the null hypothesis and conclude that there are no significant differences in those data of 2011 to 2015 and those data of 2016. However, the p-value for budget variable are less than 0.05 and far outside the confidence interval, therefore this variable is not suitable for the model application on year 2016 movies. This report believes that the reason of this inconsistent could be the limitation of the raw data. We have not included in the t-test the dummy variables because they do not follow a normal distribution (following a two-point distribution).
...
insig_variable_table <- data.frame(Statistics, budget_11_15, budget_16)
...
C. z-test for betas--to be filled
```{r}
beta_11_15<-c(0.133, -3082942, -1001729, 2486190, 31.40, ) # for gross
beta16<-
t.test(beta_11_15, beta16)
